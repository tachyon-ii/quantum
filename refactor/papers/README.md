...
- **`.md` suffixes:** `<basename>-text-only.md` is the verbatim conversion; `<basename>-READING-NOTES.md` is commentary.
- **Open decision (2026-09-03):** the `.md` files as converted drop `et-al` (`kopp-magnetic-…`, `blokker-chemical-…`, `xu-photonic-…`) while the PDFs keep it. Either is fine; pick one and rename the other side so `pdf` and `md` share a basename again. Lead-surname-only is shorter and the shelf is small enough that collisions are handled by the second-token rule — recommendation: drop `et-al` everywhere.
...
| `li-sparveris-et-al-measured-proton-electromagnetic-structure` | R. Li, N. Sparveris, et al. (JLab) | Measured proton electromagnetic structure deviates from theoretical predictions · arXiv:2210.11461 · 2022 | `converted-to-md/` | ✅ | proton form-factor anomaly — relevant to the capsid charge-radius coupling (`capsid_audit` §2.3) |
| `besiii-oscillating-features-electromagnetic-structure-neutron` | BESIII Collaboration (Ablikim et al.) | Oscillating features in the electromagnetic structure of the neutron (arXiv title: *New Features…*) · arXiv:2103.12486 / Nat. Phys. · 2021 | `converted-to-md/` | ✅ | neutron form-factor oscillations — the neutron's own "orbit" fingerprint |
| `kopp-et-al-magnetic-moments-astrophysical-neutrinos` | J. Kopp, T. Opferkuch, E. Wang | Magnetic Moments of Astrophysical Neutrinos · arXiv:2212.11287v3 · 2024 | `converted-to-md/` | ✅ | n=1 neutrino as the sole primitive ⇒ its moment is a first-class prediction target |
...
| `sen-of-mass-charge-and-spin` | D. Sen | Of Mass, Charge and Spin, the Basic Attributes of Matter — Their Physical Origin · Horizons in World Physics 275, ch. 9 · 2011 | `converted-to-md/` | ✅ | the orthodox review of exactly the three things §17.2 says must be derived |
| `blokker-et-al-chemical-bond-atom-size-vs-electronegativity` | E. Blokker, …, F. M. Bickelhaupt | The Chemical Bond: When Atom Size Instead of Electronegativity Difference Determines Trend in Bond Strength · Chem. Eur. J. 27, 15616 · 2021 | `converted-to-md/` | ✅ | size-over-electronegativity — geometry beating a dial, one floor up |
| `swart-soccer-ball-symmetry` | David Swart | Soccer Ball Symmetry · Bridges 2015 · 2015 | `converted-to-md/` | ✅ | truncated-icosahedron / capsid geometry reference |
| `xu-et-al-11-teraflops-photonic-convolutional-accelerator` | X. Xu, …, D. J. Moss (Swinburne) | 11 TOPS photonic convolutional accelerator for optical neural networks · Nature 589 · 2021 | `converted-to-md/` | ✅ | analogue compute-in-place, silicon-photonic — FUBIT-adjacent, not physics-spine |
| `schrodinger-what-is-life-mind-and-matter` | E. Schrödinger (fwd. R. Penrose) | What is Life? · Mind and Matter · Autobiographical Sketches · CUP | `converted-to-md/` | ✅ | the estate's namesake shelf; Axiom I is a descendant of the aperiodic-crystal chapter |

**Conversion status: 11 of 11 converted** (James, 2026-09-03 — "same content input, less tokens"). `pdf/` is empty; everything is in `pdf/converted-to-md/` with its `md/…-text-only.md`. Read the `.md`, not the PDF. Note: BESIII has two `.md` files (`…-new-features-…-text-only.md`, the arXiv-title verbatim, and `…-oscillating-features-….md`) — one is redundant; James to pick.

## Cited but not on the shelf

- ~~"Helland et al. (2026)"~~ — **resolved 2026-09-03:** it is the estate's own forgetting paper, `../docs/v9-forgetting.md` (Helland, Pokhrel, Shannon†, Cooper†, Freeman — *Forgetting Is All You Need*). Not a shelf item; a docs item. The review cited us.
