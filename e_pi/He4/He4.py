...
class TetrahedronSimulation:
    def __init__(self, disc_radius=0.5):
        ...
        self.disc_data = self.calculate_disc_data()
    ...
    def get_orthogonal_plane_with_gradient(self, p1, p2, centroid):
        ...
        for r_idx in range(n_radial):
            ...
            for t_idx in range(n_angular - 1):
                ...
                p1_quad = centroid + r_inner * (np.cos(t1) * u1 + np.sin(t1) * u2)
                p2_quad = centroid + r_outer * (np.cos(t1) * u1 + np.sin(t1) * u2)
                p3_quad = centroid + r_outer * (np.cos(t2) * u1 + np.sin(t2) * u2)
                p4_quad = centroid + r_inner * (np.cos(t2) * u1 + np.sin(t2) * u2)
        ...
        theta = np.linspace(0, 2*np.pi, n_angular)
        for t in theta:
            point = centroid + self.disc_radius * (np.cos(t) * u1 + np.sin(t) * u2)
            outer_edge.append(point)
        
        return polygons, colors, np.array(outer_edge)
    
    def calculate_disc_data(self):
        """Pre-calculate disc vertices for all planes"""
    ...
    def plot_structure(self):
        ...
        # Recalculate disc data if opacity distance changed
        self.disc_data = self.calculate_disc_data()
        ...
        for polygons, colors, edge, label in self.disc_data:
    ...
    def setup_ui(self):
        ...
        rax = plt.axes([0.02, 0.5, 0.12, 0.1])
        self.radio = RadioButtons(rax, ('Disc', 'Wireframe'))
        ...
        self.radio.on_clicked(self.update_render_mode)
