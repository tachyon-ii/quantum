...
def bin_analysis(meta_data, bin_width=0.02):
...
def plot_meta_analysis(meta_data, binned, periods, powers):
    ...
    # Find peaks in the phase-folded data
    from scipy.signal import find_peaks
    peaks, properties = find_peaks(phase_means_valid, prominence=0.01)
    
    # Mark the peaks
    if len(peaks) > 0:
        ax5.plot(phase_centers_valid[peaks], phase_means_valid[peaks], 'go', markersize=8, label='Peaks')
        
        # Calculate inter-peak distances
        peak_phases = phase_centers_valid[peaks]
        if len(peak_phases) > 1:
            ...
            for i, p in enumerate(peak_phases):
                print(f"  Peak {i+1}: phase = {p:.3f} (x equivalent = {p*0.167:.3f})")
            
            print("\nInter-peak distances:")
            for i, d in enumerate(inter_peak_distances):
                print(f"  Peak {i+1} to {i+2}: {d:.3f} phase units ({d*0.167:.3f} in x)")
            
            print(f"\nMean inter-peak distance: {np.mean(inter_peak_distances):.3f} phase units")
            print(f"                         = {np.mean(inter_peak_distances)*0.167:.3f} in x units")
...
def main():
    ...
    # Bin the data
    binned = bin_analysis(meta_data, bin_width=0.02)
    print(f"Binned into {len(binned['x'])} bins")
    ...
    # Create plots
    fig = plot_meta_analysis(meta_data, binned, periods, powers)
