...
class CMBPeakMapper:
    ...
    def generate_synthetic_cmb_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        using James Freeman's exact original parameters
        """
        # Multipole range
        ...
        # James Freeman's ORIGINAL acoustic peaks (DO NOT MODIFY)
        peak1_pos, peak1_amp = 220, 5800  # P1: Largest (W1 cascade)
        peak2_pos, peak2_amp = 530, 4200  # P2: Medium (W2 freeze)  
        peak3_pos, peak3_amp = 800, 2800  # P3: Smaller (W3 det-cord)
        
        # Build synthetic spectrum
        power = np.zeros_like(ell)
        
        # Low-ℓ plateau (ISW effect) - James's original parameters
        power += 1000 * np.exp(-(ell - 10)**2 / (2 * 30**2))
        
        # Primary acoustic peaks with Gaussian profiles - James's original
        power += peak1_amp * np.exp(-(ell - peak1_pos)**2 / (2 * 50**2))
        power += peak2_amp * np.exp(-(ell - peak2_pos)**2 / (2 * 70**2))  
        power += peak3_amp * np.exp(-(ell - peak3_pos)**2 / (2 * 80**2))
        
        # Additional smaller peaks - James's original
        power += 1800 * np.exp(-(ell - 1150)**2 / (2 * 90**2))
        power += 1200 * np.exp(-(ell - 1480)**2 / (2 * 100**2))
        
        # High-ℓ damping (Silk damping) - James's original
        damping = np.exp(-(ell/1400)**1.5)
        power *= damping
        
        # Add realistic noise - James's original level
        noise_level = 0.02 * np.max(power)
        power += np.random.normal(0, noise_level, len(power))
        
        self.multipoles = ell
        self.power_spectrum = power
        
        print(f"Generated synthetic CMB data with Freeman's ORIGINAL predicted peak structure")
