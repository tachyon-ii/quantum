...
def dipole_wave_system():
    ...
    def calculate_wave_sets(x, angle_deg=0, separation_factor=1.0):
        """
        Two wave sources separated by separation_factor * λ (for each wavelength component),
        both π radians out of phase, viewed from angles around their centroid
        
        angle_deg: viewing angle from centroid
        separation_factor: multiplier for wavelength separation (1.0 = λ, 0.75 = 0.75λ, etc.)
        ...
        for n in range(7):
            ...
            # Sources separated by (separation_factor * λ)/2 on each side of centroid
            separation_distance = (separation_factor * wavelength) / 4
            source1_position = -separation_distance  # to the left of centroid
            source2_position = +separation_distance   # to the right of centroid
    ...
    def plot_dipole_system(angle_deg=0, separation_factor=1.0):
        """
        Plot the dipole system at a specific viewing angle and separation
        """
        wave_set_1, wave_set_2, total_1, total_2 = calculate_wave_sets(x_range, angle_deg, separation_factor)
        
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle(f'Dipole System: {separation_factor}λ separation, Viewing Angle = {angle_deg}°\n(0°=Superposition, 180°=Dipole)', fontsize=16)
        ...
        ax2.set_title(f'Source 2 (π phase + {separation_factor}λ separation)')
    ...
    def analyze_angle_sweep():
        ...
        for three different separation distances: 0.75λ, 1.0λ, and 1.25λ
        """
        angles = np.linspace(0, 180, 19)  # Every 10 degrees from 0° to 180°
        separations = [0.75, 1.0, 1.25]  # 0.75λ, 1.0λ, 1.25λ
        separation_labels = ['0.75λ', '1.0λ', '1.25λ']
        colors_sep = ['red', 'blue', 'green']
        
        results = {}
        
        for sep_idx, sep_factor in enumerate(separations):
            max_amplitudes = []
            rms_amplitudes = []
            
            print(f"\nAngle sweep analysis for {separation_labels[sep_idx]} separation:")
            print("Angle\tMax Amplitude\tRMS Amplitude")
            print("-" * 40)
            
            for angle in angles:
                _, _, total_1, total_2 = calculate_wave_sets(x_range, angle, sep_factor)
                combined = total_1 + total_2
                ...
                if angle % 30 == 0:  # Print every 30 degrees
                    print(f"{angle:3.0f}°\t{max_amp:.3f}\t\t{rms_amp:.3f}")
            
            results[separation_labels[sep_idx]] = {
                'max_amplitudes': max_amplitudes,
                'rms_amplitudes': rms_amplitudes
            }
        
        # Plot comparison of all three separations
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        
        for sep_idx, sep_label in enumerate(separation_labels):
            ax1.plot(angles, results[sep_label]['max_amplitudes'], 
                    color=colors_sep[sep_idx], marker='o', linewidth=2, markersize=3, 
                    label=f'{sep_label} separation')
        
        ax1.set_xlabel('Viewing Angle (degrees)')
        ax1.set_ylabel('Maximum Amplitude')
        ax1.set_title('Maximum Amplitude vs Viewing Angle\nfor Different Separations')
        ax1.grid(True, alpha=0.3)
        ax1.set_facecolor('black')
        ax1.axvline(x=0, color='white', linestyle='--', alpha=0.5, label='Superposition')
        ax1.axvline(x=180, color='white', linestyle=':', alpha=0.5, label='Dipole')
        ax1.legend()
        
        for sep_idx, sep_label in enumerate(separation_labels):
            ax2.plot(angles, results[sep_label]['rms_amplitudes'], 
                    color=colors_sep[sep_idx], marker='o', linewidth=2, markersize=3,
                    label=f'{sep_label} separation')
        
        ax2.set_xlabel('Viewing Angle (degrees)')
        ax2.set_ylabel('RMS Amplitude')
        ax2.set_title('RMS Amplitude vs Viewing Angle\nfor Different Separations')
        ax2.grid(True, alpha=0.3)
        ax2.set_facecolor('black')
        ax2.axvline(x=0, color='white', linestyle='--', alpha=0.5, label='Superposition')
        ax2.axvline(x=180, color='white', linestyle=':', alpha=0.5, label='Dipole')
        ax2.legend()
    ...
    # Plot at key angles for different separations
    key_angles = [0, 45, 90, 135, 180]
    separations = [0.75, 1.0, 1.25]
    
    # Show one example plot for each separation at 90 degrees
    for sep_factor in separations:
        fig = plot_dipole_system(90, sep_factor)
        plt.show()
    
    # Analyze full angle sweep for all separations
    angles, results = analyze_angle_sweep()
