...
def generate_wave_time_series():
    ...
    os.makedirs('wave_frames', exist_ok=True)
    
    # Generate frames in simple sequential order
    for frame_idx, t in enumerate(time_steps):
        # Generate composite version
        generate_frame(frame_idx, t, x_range, colors, show_composite=True)
        
        # Generate individual waves version
        generate_frame(frame_idx, t, x_range, colors, show_composite=False)
        
        print(f"Generated frame {frame_idx + 1}/{num_frames}")
    
    print(f"\nGenerated {num_frames} frames in 'wave_frames/' and 'wave_frames_individual/' directories")
    print("Composite version: convert -delay 20 wave_frames/*.png wave_animation.gif")
    print("Individual version: convert -delay 20 wave_frames_individual/*.png wave_individual.gif")
    
    # Create contact sheets for both versions
    create_contact_sheet(num_frames, 'wave_frames', 'wave_contact_sheet.png', 'Composite')
    create_contact_sheet(num_frames, 'wave_frames_individual', 'wave_contact_sheet_individual.png', 'Individual')

def generate_frame(frame_idx, t, x_range, colors, show_composite=True):
    ...
    for n in range(7):  # n=0 to n=6
        ...
        if show_composite:
            ...
            # Plot individual wave (full opacity on red background)
    ...
    ax_main.set_title(f'Wave Superposition: λ = e^n/π {title_suffix} (t = {t:.2f})', 
    ...
    if show_composite:
        os.makedirs('wave_frames', exist_ok=True)
        filename = f'wave_frames/frame_{frame_idx:03d}.png'
    else:
        os.makedirs('wave_frames_individual', exist_ok=True)
        filename = f'wave_frames_individual/frame_{frame_idx:03d}.png'
...
def create_contact_sheet(num_frames, frames_dir, output_filename, version_name):
    ...
    # Fix the frame placement - explicitly map label position to correct frame
    for label_num in range(1, num_frames + 1):  # Labels go 1,2,3...20
        frame_num = label_num - 1  # Frame files go 0,1,2...19
        row = (label_num - 1) // cols  # Which row this label should be in
        col = (label_num - 1) % cols   # Which column this label should be in
        
        # Load and display the correct frame for this label position
        try:
            img = plt.imread(f'{frames_dir}/frame_{frame_num:03d}.png')
            axes[row, col].imshow(img)
            axes[row, col].set_title(f'Frame {label_num}', fontsize=10)
            axes[row, col].axis('off')
        except FileNotFoundError:
            axes[row, col].text(0.5, 0.5, f'Frame {label_num}\nNot Found', 
