...
"""
Unified Nuclear Binding Model v3
================================

This script unifies the lambda–d scan baseline (v1) and the advanced channel-split,
geometry-aware optimizer (v2) into a single configurable tool.

Core principles carried forward:
- Exact deuteron anchoring (solve g^2(1,0) inside the objective when optimizing).
- Spin–isospin channel splitting for np pairs, proper handling for pp edges.
- Unified soft Coulomb for pp with finite-size rc_pp.
- Minimal 3-body "face" term using altitude, with optional short-range repulsion.
- A-dependent kinetic term (Gaussian normal-mode proxy).
- Optional vertex phase-matching penalty.
- Clean acceptance checks + one-page report and plots.

Quick usage:
    python nuclear_unified_v3.py --mode scan --lam-min 1.30 --lam-max 1.38 --d-min 1.2 --d-max 2.4
    python nuclear_unified_v3.py --mode optimize
    python nuclear_unified_v3.py --help

Outputs (by default):
  - v3_scan.csv, v3_top_candidates.csv, v3_heatmaps.png (scan mode)
  - v3_results.csv, v3_params.json, v3_complete.png (optimize mode)

Author: Schrödinger-mode assistant
License: MIT
"""
...
class Geometry:
    A: int
    a: float
    Z: int = 0  # number of protons for A=3; ignored for A=2 and A=4
    ...
    def edges(self) -> List[Tuple[int, int, bool]]:
        ...
        if self.A == 2:
            ...
            # Equilateral triangle, label vertices 0..2, Z protons starting at 0..Z-1
...
class ModelParams:
    ...
    g2: Dict[Tuple[int, int], float] = field(default_factory=lambda: {
        (1, 0): 100.0,  # (S=1, T=0) — will be solved to anchor deuteron
        (0, 1): 75.0,
        (1, 1): 60.0,
        (0, 0): 45.0,
    })
...
def sum_pair_energy(geom: Geometry, P: ModelParams) -> float:
    ...
    for (i, j, is_pp) in geom.edges():
        ...
        for (S, T), w in wt.items():
            ...
            # Apply tensor scaling to attraction (g2)
...
def phase_mismatch_energy(geom: Geometry, P: ModelParams) -> float:
    if not P.phase_enable:
        return 0.0
    # For ideal regular triangle (deg=2) and tetrahedron (deg=3), no penalty.
    # Deviation proxy kept zero for these regular geometries.
    return 0.0
...
def solve_deuteron_g2(target_E, d_range, P: ModelParams):
    """Solve g^2(1,0) so that the deuteron minimum equals target_E; return (g2_10, d*, E*)."""
    def residual(g2_10):
...
# -----------------------------------------------------------------------------
# Scan mode (lambda–d baseline with minimal 3-body fit to He-4)
# -----------------------------------------------------------------------------

def run_scan(lam_vals: np.ndarray, d_vals: np.ndarray, lam_r: float = 0.5) -> pd.DataFrame:
    rows = []
    for lam in lam_vals:
        for d in d_vals:
            ...
            if g2 is None or g2r is None or g2 <= 0 or g2r <= 0:
                continue
            P = ModelParams(lam=lam, lam_r=lam_r, g2={(1,0): g2, (0,1): g2*0.75, (1,1): g2*0.60, (0,0): g2*0.45},
                            g2r_all=g2r, rc_pp=0.90, three_body_enable=True, W=12.0, Lam3=lam, Wr=0.0, Lam3r=0.7,
...
def solve_g2_g2r_closed_form(d: float, lam: float, lam_r: float):
    # Algebraic two-equation solve (stationarity + energy at D)
    try:
        eL = math.exp(-d/lam) / d
...
def objective(theta: Dict[str, float], base: ModelParams):
    ...
    split = E_3He - E_3H
    # Loss
    w = {"D": 0.01, "4He": 10.0, "3H": 1.0, "3He": 1.0, "split": 2.0}
    loss = 0.0
    ...
    res = {
        ...
        "split": split, "g2_10": g2_10,
    }
    return loss, res, P

def optimize(base: ModelParams, max_iter: int = 80):
    theta = {
        "g2_01": base.g2[(0,1)],
        "g2_11": base.g2[(1,1)],
        "g2_00": base.g2[(0,0)],
    ...
    scales = {
        "g2_01": 5.0, "g2_11": 5.0, "g2_00": 5.0,
        "W": 1.0, "Lam3": 0.03, "Wr": 0.4,
        "c3": 0.01, "k_phi": 0.2,
    }
    bounds = {
        "g2_01": (40, 100), "g2_11": (30, 80), "g2_00": (20, 60),
    ...
    for it in range(max_iter):
        ...
        for k in list(theta.keys()):
            for sign in (+1, -1):
                ...
                step = scales[k] * sign
                lo, hi = bounds[k]
                trial[k] = float(np.clip(trial[k] + step, lo, hi))
...
def plot_scan_heatmaps(df: pd.DataFrame, best_row: pd.Series, out_png: str):
    ...
    quantities = {
        ...
        'He3_minus_H3_MeV': 'He-3 minus H-3 split (MeV)',
        'g2_MeVfm': 'g^2 Attractive (MeV·fm)',
        'g2r_MeVfm': 'g^2_r Repulsive (MeV·fm)'
    }
    fig, axes = plt.subplots(2, 3, figsize=(16, 10))
    fig.suptitle('lambda–d Parameter Space (v3 scan)')
    for idx, (key, title) in enumerate(quantities.items()):
        ...
        ax.set_xlabel('lambda (fm)'); ax.set_ylabel('d* (fm)'); ax.set_title(title)
...
def plot_complete(optP: ModelParams, res: Dict[str, float], out_png: str):
    ...
    bars = ax3.bar(ch, st)
    ax3.set_xlabel('Channel (S,T)'); ax3.set_ylabel('g^2 (MeV·fm)'); ax3.set_title('Channel Strengths')
    ...
    ax4.set_xlabel('Edge length (fm)'); ax4.set_ylabel('Energy (MeV)'); ax4.set_title('4He Components')
    ...
    ax5.plot(a_scan, splits, linewidth=2, label='3He-3H')
    ...
    fig.suptitle('Unified Nuclear Binding Model v3')
    plt.tight_layout()
    plt.savefig(out_png, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
def main():
    ap = argparse.ArgumentParser(description="Unified Nuclear Binding Model v3")
    ap.add_argument("--mode", choices=["scan", "optimize"], default="scan",
                    help="scan: lambda–d baseline; optimize: full channel-split optimization")
    # Scan options
    ...
    if args.mode == "scan":
        ...
        df = run_scan(lam_vals, d_vals, lam_r=0.5)
        ...
        df['W_abs_norm'] = norm_col(df['W_abs'])
        df['He4_a_deviation_norm'] = norm_col(df['He4_a_deviation'])
        df['split_error_norm'] = norm_col(df['split_error'])
        ...
        print(top.iloc[0][['lambda_fm','d*_fm','W_MeVfm','He4_a*_fm','He3_minus_H3_MeV']])
    else:
        base = ModelParams()
        ...
        with open("v3_params.json","w") as f:
            json.dump({
                "lambda": base.lam, "lambda_r": base.lam_r, "rc_pp": base.rc_pp,
