...
class BerylliumInterference:
    def __init__(self):
        ...
        print("MODELING 4 ELECTRONS: Beryllium Configuration")
        print("3 electrons already positioned + 1 additional electron")
    ...
    def calculate_beryllium_field(self, fourth_electron_radius, x_range=(-8, 8), y_range=(-8, 8), resolution=320, n_angles=N_ANGLE_SAMPLES):
        """
        Calculate the interference field experienced by a 4th electron at given radius
        from the existing 3-electron configuration
        """
        ...
        # Sample different orientations of the existing 4-electron pairs
        angles = np.linspace(0, 2*np.pi, n_angles, endpoint=False)
        
        print(f"Calculating Be field with 4th electron at r={fourth_electron_radius}...")
        
        for i, theta in enumerate(angles):
            # 3-electron configuration: 2 at ±1,0 + 1 at optimal Li position
            ...
            electron_3 = np.array([3.1 * np.cos(theta + np.pi/2), 3.1 * np.sin(theta + np.pi/2)])     # at Li position r=3.1
            
            # Calculate waves from the existing 3-electron configuration
            ...
            wave_3 = self.calculate_wave_from_source(X, Y, electron_3, phase_multiplier=1.0)   # Li electron
            
            # Interference field from the existing 3 electrons (this is what the 4th electron "sees")
            three_electron_field = wave_1 + wave_2 + wave_3
            
            # Probability weight for this orientation
            weight = self.calculate_probability_weight(theta)
            
            # Add to weighted average
            total_interference += weight * three_electron_field
            total_weight += weight
    ...
    def find_optimal_fourth_electron_position(self):
        """
        Find where a 4th electron would naturally position itself
        ...
        print("Finding optimal position for 4th electron...")
        
        for r in test_radii:
            # Calculate the field at this radius (average around the circle)
            X, Y, field = self.calculate_beryllium_field(r, resolution=160, n_angles=720)  # Lower resolution for speed
    ...
    def plot_beryllium_analysis(self):
        """
        Plot the complete Beryllium analysis
        """
        # Find optimal position for 4th electron
        radii, energies = self.find_optimal_fourth_electron_position()
        
        # Create the plot
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(18, 7))
        
        # Plot 1: Field experienced by 4th electron
        X, Y, field = self.calculate_beryllium_field(2.0)  # Show field for electron at r=2
        ...
        ax1.set_title(f'Field Experienced by 4th Electron\n(from 3-electron configuration)')
        ...
        # Plot 2: Energy vs radius for 4th electron
        ax2.plot(radii, energies, 'purple', linewidth=3, label='4th Electron Energy')
        ...
        ax2.scatter(optimal_radius, min_energy, color='purple', s=100, zorder=5, 
                   label=f'Optimal position: r={optimal_radius:.1f}')
        
        ax2.set_title(f'Energy Landscape for 4th Electron\nBeryllium Configuration')
        ...
        # Analysis
        print(f"\n=== BERYLLIUM (4th ELECTRON) ANALYSIS ===")
        print(f"3 electrons already positioned: 2 at Shell 1, 1 at Li position")
        print(f"4th electron optimal position: r = {optimal_radius:.2f} Bohr")
        print(f"Minimum energy: {min_energy:.4f}")
        
        # Compare to known shell positions
        if optimal_radius < 2.0:
            print("→ 4th electron prefers to stay near Shell 1")
        elif 2.0 <= optimal_radius < 5.0:
            print("→ 4th electron forced to Shell 2 region")
        else:
            print("→ 4th electron pushed to outer shells")
...
def main():
    """Generate Beryllium interference analysis"""
    
    wave_sim = BerylliumInterference()
    
    print("=== BERYLLIUM (4th ELECTRON) WAVE INTERFERENCE ===")
    print("Configuration: 3 electrons already positioned")
    print("Question: Where does the 4th electron naturally position itself?")
    print("Expected: Pushed closer to Shell 2 than Li 3rd electron")
    
    radii, energies, optimal_radius = wave_sim.plot_beryllium_analysis()
