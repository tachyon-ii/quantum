...
"""
Unified Nuclear Binding Model v3.5 (geometry-aware for A=3)
===========================================================

Key addition:
- A=3 can be either a compact **triangle** (3 edges) or a **chain/V** (2 edges, no 0–2 edge).
- Per-isotope layouts:
    * ³H (N–P–N)  : default triangle (symmetric) OR chain with proton centered.
    * ³He (P–N–P) : default chain with proton indices at the ends (no pp edge).
- He-4 remains tetrahedral; deuteron is a single edge.

CLI flags:
  --a3-h3-geom {triangle,chain}
  --a3-he3-geom {triangle,chain}

Usage examples:
  python3 nuclear_unified_v3_5_geom.py --mode scan --a3-h3-geom triangle --a3-he3-geom chain
  python3 nuclear_unified_v3_5_geom.py --mode optimize --iters 150 --a3-h3-geom triangle --a3-he3-geom chain
"""
...
class Geometry:
    A: int
    a: float
    geom_type: Optional[str] = None  # for A=3: 'triangle' or 'chain'
    proton_idx: Optional[Set[int]] = None  # explicit proton indices for pp detection

    def edges(self) -> List[Tuple[int, int, bool]]:
        """Return list of (i, j, is_pp) edges based on topology and proton indices."""
        if self.A == 2:
            # Deuteron: one edge (np), no pp
            return [(0, 1, False)]
        elif self.A == 3:
            # Define edges by topology
            if self.geom_type == 'chain':
                pairs = [(0,1), (1,2)]
            else:
                pairs = [(0,1), (1,2), (2,0)]
            pset = self.proton_idx if self.proton_idx is not None else set()
            out = []
            for i,j in pairs:
                is_pp = (i in pset) and (j in pset)
                out.append((i,j,is_pp))
            return out
        elif self.A == 4:
            # Complete tetrahedral graph (6 edges). Protons default {0,1}
            pairs = []
            for i in range(4):
                for j in range(i+1,4):
                    pairs.append((i,j))
            pset = self.proton_idx if self.proton_idx is not None else {0,1}
            out = []
            for i,j in pairs:
                is_pp = (i in pset) and (j in pset)
                out.append((i,j,is_pp))
    ...
    def face_altitudes(self) -> List[float]:
        """Return list of face altitudes:
           - A=3 triangle has one face; chain has none.
           - A=4 tetrahedron has 4 faces.
        """
        if self.A == 2:
            return []
        if self.A == 3:
...
def make_geom_A3(isotope: str, a: float, geom_type: str) -> Geometry:
    """Construct A=3 geometry with proton indices arranged per isotope & topology.

    Index mapping (nodes 0-1-2):
      - For chain:
          * He3 (P-N-P): protons at ends -> proton_idx={0,2}
          * H3  (N-P-N): proton in center -> proton_idx={1}
      - For triangle:
          * Symmetry: choose proton_idx={0,1} for He3; {0} for H3 (pp detection present for He3)
    """
    iso = isotope.strip().lower()
    if geom_type not in ('triangle','chain'):
        geom_type = 'triangle'
    if iso == 'he3':
        if geom_type == 'chain':
            pset = {0,2}  # P-N-P: no pp edge
        else:
            pset = {0,1}  # triangle: one pp edge present
    elif iso in ('h3','t','tritium','3h'):
        if geom_type == 'chain':
            pset = {1}    # N-P-N
        else:
            pset = {0}    # triangle with one proton somewhere (no pp edge anyway)
    else:
        # default to He3 mapping if unknown
        pset = {0,2} if geom_type=='chain' else {0,1}
    return Geometry(A=3, a=a, geom_type=geom_type, proton_idx=pset)

def edges_per_nucleon(A: int) -> int:
    """X2=1, X3=2 (triangle), X4=3 edges per nucleon (chain has 2 total edges)."""
    if A == 2: return 1
    if A == 3: return 2
    if A == 4: return 3
...
class ModelParams:
    ...
    # Soft Coulomb
    rc_pp: float = 1.25
    # Optional A=3 Coulomb screening multiplier (kept for compatibility; set 1.0 to disable)
    sigma_pp_A3: float = 1.0
    # Channel strengths g^2(S,T); repulsive core shared
...
def pair_potential_central(r: float, S: int, T: int, is_pp: bool, A: int, P: ModelParams) -> float:
    r = max(1e-12, float(r))
...
def sum_pair_energy(geom: Geometry, P: ModelParams) -> float:
    ...
    for (i, j, is_pp) in geom.edges():
        ...
        for (S, T), w in wt.items():
            ...
            V = pair_potential_central(a, S, T, is_pp, A, P)
            # apply S=1 tensor proxy
            if S == 1:
                V *= tf
            edge_sum += w * V
        total += edge_sum
...
# -----------------------------------------------------------------------------
# Scan mode (λ–d baseline; He-4 fit W; A=3 geometries per isotope)
# -----------------------------------------------------------------------------
...
def run_scan(lam_vals: np.ndarray, d_vals: np.ndarray, lam_r: float, rc_pp: float,
             a3_h3_geom: str, a3_he3_geom: str) -> pd.DataFrame:
    rows = []
    for lam in lam_vals:
        for d in d_vals:
            ...
            P = ModelParams(lam=lam, lam_r=lam_r,
                            ...
                            g2r_all=g2r, rc_pp=rc_pp, sigma_pp_A3=1.0,
                            three_body_enable=True, W=12.0, Lam3=lam, Wr=0.0, Lam3r=0.7,
                            tensor_enable=True, eta_T=0.0)
            # He-4 pairs-only minimum
            a0, E0 = minimize_1d(lambda a: energy_total(Geometry(4, a, proton_idx={0,1}), P)[0], lo=0.8, hi=2.5)
            ...
            # A=3 predictions with chosen geometries
            a3H, E3H = minimize_1d(lambda a: energy_total(make_geom_A3('H3', a, a3_h3_geom), P)[0], lo=0.8, hi=2.5)
            a3He,E3He= minimize_1d(lambda a: energy_total(make_geom_A3('He3',a, a3_he3_geom),P)[0], lo=0.8, hi=2.5)

            rows.append({
                "lambda_fm": lam, "d*_fm": d, "rc_pp_fm": rc_pp,
                "a3_h3_geom": a3_h3_geom, "a3_he3_geom": a3_he3_geom,
                "g2_MeVfm": g2, "g2r_MeVfm": g2r,
                "He4_a*_fm": a0, "He4_E_MeV": TARGETS["4He"], "W_MeVfm": W,
...
# -----------------------------------------------------------------------------
# Optimize mode (full model; D solved; A=3 geometries per isotope)
# -----------------------------------------------------------------------------

def objective(theta: Dict[str, float], base: ModelParams, a3_h3_geom: str, a3_he3_geom: str):
    ...
    P.sigma_pp_A3 = theta.get("sigma_pp_A3", P.sigma_pp_A3)

    # Solve deuteron exactly
    g2_10, d_star, E_d = solve_deuteron_g2(TARGETS["D"], (1.2, 1.9), P)

    # A=3 energies with chosen geometries
    a3H,  E_3H  = minimize_1d(lambda a: energy_total(make_geom_A3('H3', a, a3_h3_geom),  P)[0], lo=0.8, hi=2.5)
    a3He, E_3He = minimize_1d(lambda a: energy_total(make_geom_A3('He3',a, a3_he3_geom), P)[0], lo=0.8, hi=2.5)

    # He-4
    a4, E_4He = minimize_1d(lambda a: energy_total(Geometry(4, a, proton_idx={0,1}), P)[0], lo=0.8, hi=2.5)
    split = E_3He - E_3H

    # Loss
    w = {"D": 0.01, "4He": 10.0, "3H": 1.0, "3He": 1.0, "split": 4.0}
    loss = 0.0
    loss += w["D"] * (E_d - TARGETS["D"])**2
    loss += w["4He"] * (E_4He - TARGETS["4He"])**2
    loss += w["3H"] * (E_3H - TARGETS["3H"])**2
    loss += w["3He"] * (E_3He - TARGETS["3He"])**2
    loss += w["split"] * (split - SPLIT_EXP)**2
    ...
    res = {
        ...
        "split": split, "g2_10": g2_10, "rc_pp": P.rc_pp, "sigma_pp_A3": P.sigma_pp_A3,
    }
    return loss, res, P

def optimize(base: ModelParams, max_iter: int, a3_h3_geom: str, a3_he3_geom: str):
    theta = {
        "g2_01": base.g2[(0,1)],
        "g2_11": base.g2[(1,1)],
        "g2_00": base.g2[(0,0)],
        "W": base.W, "Lam3": base.Lam3, "Wr": base.Wr,
        "c3": base.c3, "rc_pp": base.rc_pp, "sigma_pp_A3": base.sigma_pp_A3,
    }
    scales = {"g2_01":5.0, "g2_11":5.0, "g2_00":5.0, "W":1.0, "Lam3":0.03, "Wr":0.4,
              "c3":0.01, "rc_pp":0.04, "sigma_pp_A3":0.05}
    bounds = {"g2_01":(40,100), "g2_11":(30,80), "g2_00":(20,60),
              "W":(8,18), "Lam3":(1.20,1.60), "Wr":(0.5,4.0),
              "c3":(0.82,0.92), "rc_pp":(1.00,1.60), "sigma_pp_A3":(0.2,1.2)}
    best_loss, best_res, best_P = objective(theta, base, a3_h3_geom, a3_he3_geom)
    for _ in range(max_iter):
        ...
        for k in list(theta.keys()):
            for sign in (+1, -1):
                ...
                loss, res, Ptrial = objective(trial, base, a3_h3_geom, a3_he3_geom)
                if loss < best_loss:
...
def plot_scan_heatmaps(df: pd.DataFrame, best_row: pd.Series, out_png: str):
    ...
    fig.suptitle('λ–d Parameter Space (v3.5 scan)')
...
def plot_complete(optP: ModelParams, res: Dict[str, float], out_png: str,
                  a3_h3_geom: str, a3_he3_geom: str):
    ...
    curves['3H'] = [energy_total(make_geom_A3('H3', a, a3_h3_geom), optP)[0] for a in a_range]
    curves['3He'] = [energy_total(make_geom_A3('He3', a, a3_he3_geom), optP)[0] for a in a_range]
    curves['4He'] = [energy_total(Geometry(4, a, proton_idx={0,1}), optP)[0] for a in a_range]
    ...
    pairs, three, kin, tot = [], [], [], []
    for a in a_he4:
        _, comp = energy_total(Geometry(4, a, proton_idx={0,1}), optP)
    ...
    # Plot 5: Split vs a (with chosen geometries)
    ax5 = fig.add_subplot(gs[1, 2])
    a_scan = np.linspace(1.0, 2.0, 50)
    splits = []
    for a in a_scan:
        E3H = energy_total(make_geom_A3('H3', a, a3_h3_geom), optP)[0]
        E3He = energy_total(make_geom_A3('He3', a, a3_he3_geom), optP)[0]
        splits.append(E3He - E3H)
    ax5.plot(a_scan, splits, linewidth=2, label='³He-³H')
    ax5.axhline(y=SPLIT_EXP)
    ax5.set_xlabel('Edge length (fm)'); ax5.set_ylabel('Split (MeV)'); ax5.set_title('Coulomb/Geometry Split')
    ...
    for (S, T) in [(1,0), (0,1), (1,1), (0,0)]:
        V = [pair_potential_central(r, S, T, False, 2, optP) for r in r_range]
    ...
    fig.suptitle(f'Unified Nuclear Binding Model v3.5 (H3={a3_h3_geom}, He3={a3_he3_geom})')
    plt.tight_layout()
    plt.savefig(out_png, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
def main():
    ap = argparse.ArgumentParser(description="Unified Nuclear Binding Model v3.5 (geometry-aware for A=3)")
    ...
    ap.add_argument("--rc-pp", type=float, default=1.20, help="pp soft-Coulomb length (fm)")
    ap.add_argument("--a3-h3-geom", choices=["triangle","chain"], default="triangle")
    ap.add_argument("--a3-he3-geom", choices=["triangle","chain"], default="chain")
    # Optimize options
    ...
    if args.mode == "scan":
        ...
        df = run_scan(lam_vals, d_vals, lam_r=0.5, rc_pp=args.rc_pp,
                      a3_h3_geom=args.a3_h3_geom, a3_he3_geom=args.a3_he3_geom)
        ...
        print("Scan complete.")
        print("Saved: v3_scan.csv, v3_top_candidates.csv, v3_heatmaps.png")
        print("Best candidate:")
        print(top.iloc[0][['lambda_fm','d*_fm','rc_pp_fm','W_MeVfm','He4_a*_fm','He3_minus_H3_MeV']])
        print("Edge multiplicity per nucleon: X2=1, X3=2 (triangle), X4=3. Chain has 2 total edges.")
    else:
        ...
        with open("v3_params.json","w") as f:
            json.dump({
                "lambda": optP.lam, "lambda_r": optP.lam_r, "rc_pp": optP.rc_pp, "sigma_pp_A3": optP.sigma_pp_A3,
        ...
        plot_complete(optP, res, "v3_complete.png", args.a3_h3_geom, args.a3_he3_geom)
        print("Optimization complete.")
        print("Pass criteria:", {"D": ok_D, "He4": ok_He4, "3H": ok_3H, "3He": ok_3He, "split": ok_split})
