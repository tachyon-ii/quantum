...
class WaveInterference2D:
    ...
    def calculate_total_interference(self, x_grid, y_grid):
        ...
        for i in range(x_grid.shape[0]):
            for j in range(x_grid.shape[1]):
                ...
                # Waves from e↑ (phase = -π, red color)
                for n in [0, 1, 2]:
                    amp_up, phase_up = self.calculate_wave_at_point(
                        point, self.e_up, self.wavelengths[n], self.energies[n], phase_offset=-np.pi
                    )
                ...
                # Waves from e↓ (phase = +π)
                for n in [0, 1, 2]:
                    amp_down, phase_down = self.calculate_wave_at_point(
                        point, self.e_down, self.wavelengths[n], self.energies[n], phase_offset=+np.pi
                    )
    ...
    def plot_interference_pattern(self, x_range=(-10, 10), y_range=(-10, 10), resolution=200):
        """
        ...
        ax1.set_yticks(range(y_range[0], y_range[1]+1))
        ax1.legend()
        ...
        ax2.set_xticks(range(x_range[0], x_range[1]+1))
        ax2.set_yticks(range(y_range[0], y_range[1]+1))
        ax2.legend()
        ...
        ax3.set_xticks(range(x_range[0], x_range[1]+1))
        ax3.set_yticks(range(y_range[0], y_range[1]+1))
        ax3.legend()
...
def main():
    ...
    print("\nSources:")
    print("  e↑ at (-1,0) - red waves, phase=-π")
    print("  N  at (0,0)  - nuclear center")  
    print("  e↓ at (+1,0) - waves, phase=+π")
